...
const Step2 = (): JSX.Element => {
  ...
  const { control, watch, setValue, getValues, handleSubmit } = useForm({
    ...
    resolver: yupResolver(yup.object({
      ...
      document: docEdit ? yup.object().when(['formEdit'], ([formEdit]) => {
        return yup.object({
          ...
          nameDocument: pattern?.autoGenerateDocName ? yup.string().nullable() : yup.string().min(3, '3').required('form.required'),
    ...
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        const freezePattern = await FilledDocumentPatternServiceClient.createAndCheckFilledDocumentPattern(
          ...
          filter(data.stages, { changeOnDraft: true }).map(stage => new DocumentPatternStage({
            ...stage,
            userOrGroups: map(stage?.userOrGroups, usr => new UserOrGroup(usr))
          })),
          []
        );
        ...
        const document = await DocumentServiceClient.createDocument(
          ...
          map(data.controlUsers, controlUser => new UserOrGroup(controlUser)),
            ...
            return concat(hash, item.contentHolderLink?.map((itm: ContentHolderLink) => {
              ...
              return new ContentHolderLink({
                ...
                contentItem: contentItem !== null ? new ContentItem({
                  ...
                  childItems: map(contentItem?.childItems, childItem => new ContentItem({
                    ...childItem,
                    value: new ContentItemValue({
                      ...childItem?.value,
                      strValue: childItem?.value?.strValue && childItem?.value?.strValue !== null ? String(childItem?.value?.strValue) : null,
                      strValue2: childItem?.value?.strValue2 && childItem?.value?.strValue2 !== null ? String(childItem?.value?.strValue2) : null,
                      hbValue: new ContentItemHBValue(childItem?.value?.hbValue)
                    }),
                  })),
                  value: new ContentItemValue({
                    ...contentItem?.value,
